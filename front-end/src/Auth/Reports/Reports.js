...
export default function Reports() {
  ...
  async function generateStudentReport(e) {
    e.preventDefault();
    if (!studentId) {
      setError('Please select or enter a student ID first.');
      return;
    }
      ...
      {activeTab === 'student' && (
        <div style={styles.sectionStack}>
          <section style={styles.card}>
            ...
            <form onSubmit={generateStudentReport} style={styles.searchForm}>
              <div style={styles.formField}>
                ...
                <select style={styles.input} value={studentId} onChange={(e) => setStudentId(e.target.value)}>
                  ...
                  {students.map((student) => (
                    <option key={student.id} value={student.id}>{optionLabelStudent(student)}</option>
                  ))}
                </select>
              </div>
              <div style={styles.formField}>
                <label style={styles.label}>Student ID</label>
                <input style={styles.input} value={studentId} onChange={(e) => setStudentId(e.target.value)} placeholder="Example: 1" />
              </div>
